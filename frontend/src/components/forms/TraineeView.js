...
function TraineeView() {
  const review = useRef();
    ...
    console.log(id);
     ...
     useEffect(()=> {
    axios
    .get(`http://localHost:8000/trainee-view?id=${id}`)
    .then((res) => {
...
const rateVal = async (event) => {
  ...
  await axios.patch(`http://localhost:8000/trainee-view?id=${id}`,{value,container,enteredReview})
...
const [value, setValue] = React.useState(0);
    ...
    >
      ...
      /></div>
        ...
        <form className={classes.form} onSubmit={rateVal} >
    ...
    <div className={classes.control}>
      ...
      <div className={classes.actions}  type='submit' >
                <button onClick={()=>{setCourseID(course._id)}}> add  </button>
            </div>
      </div>
